        ...
        `}
			...
			<nav className="bg-surface border-b border-outline-variant flex justify-between items-center px-6 h-16 w-full max-w-full z-10 shrink-0">
				<div className="flex items-center gap-8">
					<div className="text-xl font-headline font-black tracking-tighter text-primary">
						openscreenxbt
					</div>
					<div className="flex items-center gap-1 font-display font-bold text-label-lg">
						<button className="text-primary border-b-2 border-primary px-4 py-4 hover:text-primary transition-colors duration-200 scale-95 active:opacity-80">
							Launch
						</button>
						<button
							onClick={() => window.electronAPI?.switchToEditor?.()}
							className="text-on-surface-variant px-4 py-4 hover:text-primary transition-colors duration-200 scale-95 active:opacity-80 border-b-2 border-transparent"
						>
							Library
						</button>
						<button
			...
			<div className="flex flex-1 overflow-hidden relative">
				...
				<div className="flex-1 p-8 overflow-y-auto flex flex-col gap-6 relative z-10">
					<header className="flex justify-between items-end border-b border-outline pb-4">
						<div>
							<h1 className="font-headline font-black text-4xl text-on-surface tracking-tight uppercase">
								Capture Target
							</h1>
							<p className="font-label text-sm text-primary mt-2 flex items-center gap-2">
								<span
									className={`w-2 h-2 ${recording ? "bg-red-500 animate-pulse" : "bg-primary"} inline-block`}
								></span>
					...
					<div className="grid grid-cols-3 gap-4">
						...
						>
							{selectedSource && (
								<div className="absolute top-0 right-0 bg-primary text-surface font-label text-xs px-2 py-1 uppercase font-bold">
									Active
								</div>
							)}
							<span
								className={`material-symbols-outlined text-4xl mb-4 block transition-colors ${selectedSource ? "text-primary" : "text-on-surface-variant group-hover:text-on-surface"}`}
								data-icon="desktop_windows"
							>
								desktop_windows
							</span>
							<h3
								className={`font-headline font-bold text-xl mb-1 uppercase tracking-wider ${selectedSource ? "text-primary" : "text-on-surface"}`}
							>
								Select Source
							</h3>
					...
					<div className="grid grid-cols-2 gap-4 mt-4">
						<div className="bg-surface-container brutal-border p-4 flex flex-col justify-between">
							<div className="flex justify-between items-center mb-4 border-b border-outline pb-2">
								<div className="flex items-center gap-2">
									<span
										className={`material-symbols-outlined ${microphoneEnabled ? "text-on-surface" : "text-on-surface-variant"}`}
										data-icon="mic_external_on"
									>
										mic_external_on
									</span>
									<span
										className={`font-headline font-bold uppercase ${microphoneEnabled ? "text-on-surface" : "text-on-surface-variant"}`}
									>
										Audio Input
									</span>
								</div>
						...
						<div className="bg-surface-container brutal-border p-4 flex flex-col justify-between">
							<div className="flex justify-between items-center mb-4 border-b border-outline pb-2">
								<div className="flex items-center gap-2">
									<span
										className={`material-symbols-outlined ${webcamEnabled ? "text-on-surface" : "text-on-surface-variant"}`}
										data-icon="videocam"
									>
										videocam
									</span>
									<span
										className={`font-headline font-bold uppercase ${webcamEnabled ? "text-on-surface" : "text-on-surface-variant"}`}
									>
										Camera
									</span>
								</div>
							...
							<div className="flex items-center gap-4">
								<div className="font-label text-xs text-on-surface-variant">
									{webcamEnabled ? "ACTIVE SIGNAL" : "NO SIGNAL INPUT"}
								</div>
							</div>
						</div>
					</div>
